...
const Coffee = ({ singlecoffee }) => {
  ...
  const { _id, name, supplier, category, coffeeChef, taste, details, photo } =
    singlecoffee;
  ...
  const handleDelete = (_id) => {
    ...
    fetch(`https://coffee-store-server-iota-woad.vercel.app/coffee/${_id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
    ...
    <div className="bg-[#ECEAE3] flex flex-col lg:flex-row justify-center lg:justify-between space-x-3 rounded-xl p-9 rancho ">
      ...
      <div className="w-[200px] flex justify-center items-center">
        <div className="w-full flex flex-col text-lg">
          <div className="">
            {" "}
            <p className="rancho">Name: {name} </p>
          </div>
          <div className="">
            {" "}
            <p>Chef: {coffeeChef} </p>
      ...
      <div className="flex lg:flex-col space-x-5 lg:space-y-5 pt-10">
        ...
        <div className="">
          <button
            onClick={() => {
              setDelete(singlecoffee?._id);
            }}
            className="w-10 h-10 bg-red-600 hover:bg-green-700 rounded-xl"
          >
